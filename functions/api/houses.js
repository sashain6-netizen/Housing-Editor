...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (!authHeader?.startsWith('Bearer ')) {
      return new Response(JSON.stringify({ error: 'Authentication required. Please log in to create a house.' }), {
        status: 401,
        headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
      });
    }
    ...
    if (!name) {
      return new Response(JSON.stringify({ error: 'House name is required' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
      });
    }
    ...
    if (name.length > 100) {
      return new Response(JSON.stringify({ error: 'House name is too long' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
      });
    }
    ...
    await env.DB.prepare(
      'INSERT INTO houses (id, user_id, name, description, code, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)'
    ).bind(houseId, payload.userId, name, description || '', code || '', now, now).run();
    ...
    return new Response(JSON.stringify({ error: 'Failed to create your house due to a server error. Please try again.' }), {
      status: 500,
      headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
    });
  }
